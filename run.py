...
if startParam is not None:
    try:
        ...
        print('ERROR:: %s param is only number' %
              ('End' if isinstance(startParam, int) else 'Start'))
        sys.exit(1)
...
def handleLoop():
    ...
    if (imgReqErrUrl):
        ...
        if (loopType == 'param'):
            global nowParam
            if (endParam is not None and nowParam > endParam):
...
while handleLoop():
    ...
    for imgTag in imgTags:
        ...
        if ext is None:
            ...
            # already file pass
            if sameFileNamePass and os.path.isfile(
                    outputImgFileNameYetExt + ext):
                alreadyFile = True
                break
        ...
        # append img
        tempImgFileName = GET.uniqueFileName(
            outputImgFileNameYetExt + '_temp_%d' %
            len(tempImgArr), ext)
        with open(tempImgFileName, 'wb') as f:
            ...
            try:
                ...
                if (fullHeight > 65500):
                    ...
                    fullHeight -= height
                    saveImg(tempImgArr[:-1], fullWidth, fullHeight,
                            GET.uniqueFileName(outputImgFileNameYetExt, ext))
                ...
                handleLog(
                    '| ERROR:: Pillow error outputImgFileName: %s, tempImgFileName: %s' %
                    (outputImgFileNameYetExt, tempImgFileName))
